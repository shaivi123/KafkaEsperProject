...
public class StatementController {
    ...
    public ResponseEntity create(@Valid @RequestBody Statement statement) {
        ...
        try {
            ...
            EPCompiled compiled = EPLUtil.compileEPL(configuration, statement.getName(),statement.getStatement());

            EPDeployment deployment = EPLUtil.deploy(runtime, compiled);

            EPStatement s = runtime.getDeploymentService().getStatement(deployment.getModuleName(), statement.getName());
